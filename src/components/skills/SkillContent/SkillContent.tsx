...
const SkillList = [
  ...
  [
    ...
    },
    {
      ...
      title: 'TanstackQuery(ReactQuery) v3~4',
...
export default function SkillContent({ categoryIndex }: Props) {
  return SkillCategoryList.map((cate) => (
    ...
    >
      <h3 className='text-xl font-semibold text-lightest-slate leading-10 mb-6'>
      ...
      {SkillList[cate.index].map(({ description, SkillIcon, title }, index) => (
        <ul key={cate.index + '' + index}>
          <li className='flex gap-4 mb-4'>
            ...
            <div>
              <h3 className='text-xl text-lightest-slate mb-2'>{title}</h3>
